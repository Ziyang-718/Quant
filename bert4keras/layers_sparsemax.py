#! -*- coding: utf-8 -*-
# 自定义层 - 添加 TensorFlow Addons SparsemaxLoss 支持
...
from .sparsemax import sparsemax  # <--- 新增此行

# 添加 TensorFlow Addons 导入
try:
    import tensorflow_addons as tfa
    TFA_AVAILABLE = True
except ImportError:
    TFA_AVAILABLE = False
    print("Warning: TensorFlow Addons not available. SparsemaxLoss will not be available.")
    ...
    def pay_attention_to(self, inputs, mask=None, **kwargs):
        ...
        if self.normalization == 'sparsemax':
            ...
            if v_mask is not None:
                adder = (1.0 - K.cast(v_mask, K.floatx())) * -10000.0
                adder = K.expand_dims(adder, 1)
                adder = K.expand_dims(adder, 1)
                a += adder
            if a_bias is not None:
                a += a_bias
            # 调用你自定义的 sparsemax 函数
            A = sparsemax(a, axis=-1)
        else:
        # 使用原有的attention_normalize
...
# 添加新的 SparsemaxLoss 类，继承自现有的 Loss 类
class SparsemaxLoss(Loss):
    """
    自定义 Sparsemax Loss（无 TFA 依赖）

    使用公式：
    L(z, y) = 0.5 * ||sparsemax(z)||^2 - <z, y> + 0.5
    其中 y 是 one-hot label，z 是 logits，<.,.> 是点积
    """
    def __init__(self, reduction='sum_over_batch_size', **kwargs):
        super(SparsemaxLoss, self).__init__(**kwargs)
        self.reduction = reduction

    # def sparsemax(self, logits, axis=-1):
    #     z = logits
    #     z -= tf.reduce_max(z, axis=axis, keepdims=True)  # 数值稳定性处理
    #     z_sorted = tf.sort(z, direction='DESCENDING', axis=axis)
    #     z_cumsum = tf.cumsum(z_sorted, axis=axis)

    #     # 构造 k 向量
    #     k = tf.range(1, tf.shape(z)[axis] + 1, dtype=z.dtype)
    #     k_shape = tf.concat([tf.ones([tf.rank(z) - 1], dtype=tf.int32), [-1]], axis=0)
    ...
    def compute_loss(self, inputs, mask=None):
            ...
            y_true : int32, shape [batch, seq_len]
                    token id，填 -100 或 0 表示需要忽略的位置
            y_pred : float32, shape [batch, seq_len, vocab_size]
                    未做 sparsemax 前的 logits
            """
            y_true, y_pred = inputs
            vocab_size = tf.shape(y_pred)[-1]

            # 1. 先把需要忽略的位置置 0
            ignore_mask = tf.cast(y_true <= 0, y_pred.dtype)  # 0/1
            y_true = tf.where(ignore_mask > 0, tf.zeros_like(y_true), y_true)

            # 2. 整数 -> one-hot，形状 [B,L,V]
            y_true_oh = tf.one_hot(tf.cast(y_true, tf.int32),
                                depth=vocab_size,
                                dtype=y_pred.dtype)

            # 3. sparsemax 得到概率分布
            p = sparsemax(y_pred, axis=-1)                    # [B,L,V]

            # 4. 计算 sparsemax KL 损失（或用 l2-hinge）
            loss_token = 0.5 * tf.reduce_sum(
                tf.square(p - y_true_oh), axis=-1)            # [B,L]

            # 5. 屏蔽 pad/忽略位置
            loss_token *= (1.0 - ignore_mask)

            # 6. 归一化到 batch
            loss = tf.reduce_sum(loss_token) / tf.reduce_sum(1.0 - ignore_mask)
            return loss
...
# 更新 custom_objects 字典，保持原有的类名不变
custom_objects = {
    'Embedding': Embedding,
...
# 只有当 TFA 可用时才添加 SparsemaxLoss
if TFA_AVAILABLE:
    custom_objects = {
    'SparsemaxLoss': SparsemaxLoss,  # 你自己实现的类
    'sparsemax': sparsemax           # 如果你希望sparsemax也可被模型保存/加载识别
}
